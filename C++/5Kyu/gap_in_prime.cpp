...
- gap(2, 5, 7) --> [5, 7] or (5, 7) or {5, 7}
    ...
    gap(2, 5, 5) --> nil. In C++ {0, 0}. In F# [||]. In Kotlin, Dart and Prolog
    return []`
...
first pair)
    ...
    gap(6,100,110) --> nil or {0, 0} or ... : between 100 and 110 we have 101,
    103, 107, 109 but 101-107is not a 6-gap because there is 103in between and
    103-109is not a 6-gap because there is 107in between.
...
//for this kata I used geeksforgeeks to implement a prime algorithm which would
...
{
public:
    //part of the G4G algorithm-checking if number passed is prime
    //for a similar algorithm see c++/6Kyu/is_prime.cpp
    ...
    static std::pair<long long, long long> gap(int g, long long m, long long n) {
      //the vector will hold our prime numbers
      //the pair will be the values we return->initialized to zero
      std::vector<long long> prime;
      std::pair<long long, long long> numbers={0,0};
      
      //this is the algorithm on primes from geeksforgeeks
      ...
      //using our vector, the moment our condition is met, break the loop
      //if it makes it to the end with no g->return {0,0}
      int size=prime.size();
      for(int x=0;x<size-1;x++){
        if((prime[x+1]-prime[x])==g){
          numbers={prime[x],prime[x+1]};
          break;
        }
      }
