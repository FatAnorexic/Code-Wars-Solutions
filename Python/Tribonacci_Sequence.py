...
think that we would get the same sequence shifted by 2 places, but that is not the case and we 
would get:
...
def tribonacci(signature, n):
    ...
    for i in range(2, n-1):
        value=signature[i-2]+signature[i-1]+signature[i]
        signature.append(value)
        
    return signature
  
#A best practices solution may look like
def tribonacci(signature, n):
  res = signature[:n]
